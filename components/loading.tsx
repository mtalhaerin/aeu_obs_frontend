...
const Loading: React.FC<{ text?: string }> = ({ text = 'Yükleniyor...' }) => {
  ...
  if (Platform.OS === 'web') {
    try {
      ...
      logoUri = resolved?.uri ?? (localLogo as any)?.uri ?? (localLogo as any)?.default ?? undefined;
    } catch (_e) {
  ...
  return (
    <View style={styles.wrapper}>
      <View style={styles.logoWrapper}>
        {Platform.OS === 'web' ? (
          // @ts-ignore
          <img src={logoUri} alt="AEU OBS" title="AEU OBS" style={{ width: 80, height: 80, objectFit: 'contain' }} />
        ) : (
          <Image source={localLogo} style={{ width: 80, height: 80 }} resizeMode="contain" />
        )}
      </View>
